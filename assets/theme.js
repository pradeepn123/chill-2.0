(function ($) {
  ...
  */
  ...
  theme.handleBuyNow = function(element) {
    element.setAttribute("disabled", true)
    element.innerHTML = "Loading"
    setTimeout(function() {
      window.location.href = `/cart/${element.dataset.variantId}:${element.dataset.quantity}?traffic_source=buy_now`
    })
  }
  ...
  theme.icons = {
    ...
    productchevronLeft: '<svg xmlns="http://www.w3.org/2000/svg" width="23.827" height="23.827" viewBox="0 0 23.827 23.827" fill="none" stroke="currentColor" stroke-width="1" class="feather feather-chevron-left ltr-icon"><path id="arrow_forward_FILL0_wght400_GRAD0_opsz48" d="M11.913,23.827l-1.564-1.6,9.2-9.2H0V10.8H19.545l-9.2-9.2L11.913,0,23.827,11.913Z" transform="translate(23.827 23.827) rotate(180)"></path></svg>',
    productchevronRight: '<svg xmlns="http://www.w3.org/2000/svg" width="23.827" height="23.827" viewBox="0 0 23.827 23.827" fill="none" stroke="currentColor" stroke-width="1" class="feather feather-chevron-right ltr-icon"><path id="arrow_forward_FILL0_wght400_GRAD0_opsz48" d="M19.913,31.827l-1.564-1.6,9.2-9.2H8V18.8H27.545l-9.2-9.2L19.913,8,31.827,19.913Z" transform="translate(-8 -8)"></path></svg>',
  ...
  theme.applyAjaxToProductForm = function ($formContainer) {
    ...
    if (theme.settings.cart_type === 'message') {
  ...
  theme.addedToCartHandler = function (response) {
    ...
    if (theme.settings.cart_type === 'message') {
      var $cartMessageContainer = $(response.sections['cart-message']);
      var cartSummary = $cartMessageContainer.find('.cart-summary-overlay')[0];
      cartSummary.classList.add('cart-summary-overlay--off-screen');

      // show only added item
      $cartMessageContainer.find(".cart-summary-overlay__row-body:not([data-item-key=\"".concat(response.key, "\"])")).remove();

      cartSummary.querySelector('.shop-more').addEventListener('click', function (evt) {
        evt.preventDefault();
        theme.removeAddedCartMessage();
      });

      $cartMessageContainer.prependTo('body');
      setTimeout(function () {
        cartSummary.classList.remove('cart-summary-overlay--off-screen');
      }, 50);
    } else if (theme.settings.cart_type === "drawer") {
  ...
  theme.removeAddedCartMessage = function () {
    document.querySelectorAll('.cart-summary-overlay').forEach(summary => {
      summary.classList.add('cart-summary-overlay--off-screen');
      var duration = parseFloat(getComputedStyle(summary).transitionDuration) * 1000;
      setTimeout(() => $(summary.parentElement).remove(), duration);
    });
  };
  ...
  theme.LoadFilterer = function (section) {
    ...
    this.functions = {
      ...
      ajaxLoadForm: function ajaxLoadForm(evt) {
        ...
        var sort_by = new URLSearchParams(window.location.search).get("sort_by")
        var queryVals = queryVals = new URLSearchParams();
        if (sort_by) {
          queryVals.append("sort_by", sort_by)
        }
        ...
        evt.currentTarget.querySelectorAll('input, select').forEach(input => {
          ...
          input.value !== '' // has a value
              ...
              queryVals.append(input.name, input.value);
            }
        });
        // new url
  ...
  theme.initProductGallery = function (container) {
    ...
    if ($slideshow.length) {
      var slickConfig = {
        ...
        prevArrow: '<button type="button" class="slick-product-prev" aria-label="' + theme.strings.previous + '">' + theme.icons.productchevronLeft + '</button>',
        nextArrow: '<button type="button" class="slick-product-next" aria-label="' + theme.strings.next + '">' + theme.icons.productchevronRight + '</button>',
        mobileFirst: true,
  ...
  theme.CartDrawerTemplateSection = new function () {
    this.onSectionLoad = function (container) {
      this.$container = $(container);

      // listen to cart changes
      $(document).on('theme:cartchanged.cartDrawerTemplateSection', this.functions.refreshCartDependentContent.bind(this));

      // terms and conditions checkbox
      if ($('#cartform input#terms', container).length > 0) {
        $(document).on('click.cartDrawerTemplateSection', '#cartform [name="checkout"], .additional-checkout-buttons input, a[href*="/checkout"]', function () {
          if ($('#cartform input#terms:checked').length == 0) {
            alert(theme.strings.cart_terms_confirmation);
            return false;
          }
      ...
      if ($(container).data('ajax-update')) {
        $(container).on('keyup.cartDrawerTemplateSection change.cartDrawerTemplateSection changeFromButton.cartDrawerTemplateSection', '.cart-item__quantity-input', function (evt) {
          if (this.replacingContent) {
            return;
          }
          if ($(evt.currentTarget).data('initial-value') && $(evt.currentTarget).data('initial-value') == $(evt.currentTarget).val()) {
        ...
        $(container).on('click.cartDrawerTemplateSection removeFromButton.cartDrawerTemplateSection', '.remove', function (evt) {
          if (this.replacingContent) {
            return;
          }
  ...
  $(function ($) {
    ...
    theme.Sections.register('cart-drawer', theme.CartDrawerTemplateSection, { deferredLoad: false });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleResponse() {
    ...
    // scripts from age-verification section
    setTimeout(() => {
